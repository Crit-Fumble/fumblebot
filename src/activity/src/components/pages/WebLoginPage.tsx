...
export function WebLoginPage({ onLogin, testId = 'web-login-page' }: WebLoginPageProps) {
  return (
    <CenteredLayout testId={testId}>
      <div
        className="bg-discord-background-tertiary rounded-xl p-10 text-center max-w-md"
        data-testid={`${testId}-card`}
      >
        <h2
          className="text-2xl font-bold text-discord-text-normal mb-4"
          data-testid={`${testId}-title`}
        >
          Welcome to FumbleBot
        </h2>
        <p
          className="text-discord-text-muted mb-8"
        ...
        >
          You'll be redirected to Discord to authorize FumbleBot.
        </p>
      </div>
    </CenteredLayout>
  );
